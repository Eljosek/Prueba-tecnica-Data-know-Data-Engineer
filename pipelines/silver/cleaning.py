...
def conectar_sql():
    conn_str = f'DRIVER={
        DB_CONFIG["driver"]};SERVER={
        DB_CONFIG["server"]};DATABASE={
            DB_CONFIG["database"]};UID={
                DB_CONFIG["user"]};PWD={
                    DB_CONFIG["password"]}'
    try:
...
def registrar_calidad(conn, tabla, batch_id, filas_leidas, filas_limpias,
                      duplicados, nulos, fk_violaciones, duracion, estado):
    cursor = conn.cursor()
    try:
        cursor.execute("""
...
def leer_datos_bronze(nombre_tabla):
    ...
    logger.info(
        f"Leyendo {
            len(archivos_parquet)} archivo(s) Parquet de {nombre_tabla}")
...
def detectar_duplicados(df, tabla):
    ...
    logger.info(
        f"{tabla}: detectados {duplicados} duplicados (leyeron {df_before}, limpios {
            len(df)})")
...
def escribir_parquet_silver(df, nombre_tabla, batch_id):
    ...
    archivo = os.path.join(directorio_tabla,
                           f'{nombre_tabla}_clean_{batch_id}.parquet')
...
def ejecutar_silver():
    ...
    try:
        for tabla in TABLAS:
            ...
            escribir_parquet_silver(
                df, tabla, datetime.now().strftime('%Y%m%d%H%M%S'))
